...
```
 ...
 goal ─────────► │ Agent state: goal, step history, last observation, notes  │
                 ...
                 │  next_action  (choice)        │   - which tool, or FINISH
                 │  goal_met     (yes/no)        │   - is the goal answered
...
|---|---|---|---|
| `wiki_lookup` | `topic` (entity / noun phrase) | Wikipedia REST summary API (CORS, no key) | Returns the lead paragraph; main source of multi-hop facts |
| `wikidata_fact` | `entity`, `property` (fixed list: capital, population, founded, CEO…) | Wikidata API | Property is a Laya choice over a fixed list; population etc. returned as data, never reasoned by Laya |
...
| `unit_convert` | `quantity`, `to_unit` (fixed list) | Local JS | |
| `date_math` | `date`, `offset` | Local JS | "3 weeks after March 4" |
| `note_save` | `text` (clause after "remember/note that") | IndexedDB | Agent memory across sessions in the same browser |
| `note_search` | `query` (noun phrase) | MiniLM embeddings over notes (from layaAsRagJudge) | |
| `verify_claim` | `claim` (sentence) | layaAsRagJudge pipeline over notes / fetched text | Lets the agent check its own answer before finishing |
| `open_url` | `url` | fetch through CORS only; shows a notice if blocked | Optional; many sites will be blocked by CORS |
| `ask_user` | `question` (S2 or template) | UI prompt | Always available |
...
const state = {
  ...
  steps_done: [                        // last 4 steps only, one line each
    "wiki_lookup(Eiffel Tower) -> located in Paris, France"
  ],
  last_observation: "The Eiffel Tower is a wrought-iron lattice tower in Paris, France…", // truncated to ~180 tokens
  notes_hit: null
};
```
...
const actionQuestions = {
  ...
  goal_met: { type: "noul", instructions: "The information gathered so far fully answers the goal" },
...
// d.next_action = { choice: "weather", probabilities: {...}, confidence: 0.71 }
```
...
- `goal_met` above its cutoff forces `FINISH` even if `next_action` disagrees; a disagreement between the two is itself a HOLD signal.
...
| Condition | Route |
|---|---|
| `risky` = true with confidence ≥ τ_risk | **BLOCK**: user must confirm (fails closed, like the guardrail workflow) |
| action conf ≥ τ_action **and** every slot conf ≥ τ_arg **and** no slot = NONE | **AUTO** (System 1 runs the step) |
...
- `goal_met` and `next_action` disagree,
...
const s2 = await CreateMLCEngine("Qwen2.5-1.5B-Instruct-q4f16_1-MLC", { initProgressCallback });
...
const reply = await s2.chat.completions.create({
  ...
  messages: [
    ...
    { role: "user", content: JSON.stringify({
        goal: state.goal, history: state.steps_done, observation: state.last_observation,
        tools: tools.map(t => ({ id: t.id, use: t.whenToUse, slots: t.slots })),
        system1_view: { top3_actions: top3(d.next_action), slot_candidates: argQuestions } }) }
  ],
  response_format: { type: "json_object", schema: JSON.stringify(STEP_SCHEMA) }   // constrained decoding
});
```

- `STEP_SCHEMA` = `{ action: enum(tool ids), args: { slot: string }, reason: string }`. Constrained JSON keeps S2 inside the same action space as S1, so both can be compared step by step.
...
- S2 is also the only component allowed to write free text: a rewritten search query when S1 chose `NONE`, the `ask_user` question, and the final answer when the tool templates aren't enough.
...
`FINISH` builds the answer from tool result templates (for example `"Forecast for {{place}}, {{when}}: {{summary}}"`) plus the list of sources used. If S2 is loaded, a toggle lets it rewrite the templated answer into one paragraph, and the node is marked "written by S2". Optionally run `verify_claim` on the final answer before showing it (the agent checking itself with the RAG judge).
...
| `scripts/prepare_site.mjs` | Assemble `dist/` with vendored libs | from existing projects |
...
| Memory | "Remember my book club is on Thursdays" → later "When is book club?" | note_save / note_search |
| Needs rewriting | "That tall tower in Paris, how tall is it?" | S1 should pick NONE / escalate |
| Ambiguous | "Weather in Springfield" | Should HOLD or ask_user, not guess |
| Should refuse or confirm | "Delete all my notes" | risky → BLOCK |
| Stop correctly | Goals answered after one step | goal_met, no extra steps |
...
- `deploy.yml`: `npm ci` → `npm test` → `node scripts/prepare_site.mjs` → publish `dist/` (Settings → Pages → Source: GitHub Actions).
